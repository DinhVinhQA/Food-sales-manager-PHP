...
if (isset($_POST['price'])) {
    $str = 'location:all-product.php?price=' . $_POST['priceMin'] . '-' . $_POST['priceMax'];
    header($str);
}
...
$result = mysqli_query($con, "SELECT count(masanpham) as total FROM sanpham WHERE  (giakhuyenmai >= $priceMin AND giakhuyenmai <= $priceMax)");
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-9 col-md-8 col-sm-12 col-sx-12">
                <div class="row">
                    <?php
                    $sql = "SELECT * FROM sanpham WHERE (giakhuyenmai >= $priceMin AND giakhuyenmai <= $priceMax)  LIMIT $start, $limit";
                ...
                <ul class="page_pagination" style="padding-top: 40px;">
                    <li>
                        ...
                        if ($current_page > 1 && $total_page > 1) {
                            echo '<a href="all-product.php?price=' . $priceMin . '-' . $priceMax . '&&page=' . ($current_page - 1) . '" class="tran3s"><i class="fa fa-long-arrow-left" aria-hidden="true"></i></a>';
                        }
                        ...
                        for ($i = 1; $i <= $total_page; $i++) {
                            if ($i == $current_page) {
                                ...
                                echo '<a href="all-product.php?price=' . $priceMin . '-' . $priceMax . '&&page=' . $i . '">' . $i . '</a>  ';
                            }
                        }

                        if ($current_page < $total_page && $total_page > 1) {
                            echo '<a href="all-product.php?price=' . $priceMin . '-' . $priceMax . '&&page=' . ($current_page + 1) . ' " class="tran3s"><i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>  ';
                        }
                        ?>
                    </li>
                </ul>
